...
좋은 기술 문서를 작성할 땐 문서 구조를 가장 먼저 고려해야 돼요.
...
AI는 스스로 문서의 주제나 범위를 판단할 수 없어요. 작성자가 구조를 제시하지 않으면 꼭 필요한 정보가 빠지거나, 정보를 다루는 범위가 지나치게 넓어질 수 있어요. 그렇게 되면 독자가 문서의 핵심 주제를 이해하기 어려워져요.

그러면 문서 구조를 직접 설계하고, AI로 각 내용을 작성하는 법을 알아볼게요.
...
이 문서를 읽은 **독자에게 전달하고자 하는 목표**를 한 문장으로 써보세요. 
...
목적을 정의했다면, 목적을 달성하기에 가장 적합한 문서 유형을 선택하세요.
...
**문제 해결을 위한 문서**를 선택하세요. 이미 기술을 사용해 본 독자가 오류나 예외 상황을 해결하려고 할 때 도움이 돼요.
How-to 가이드나 트러블 슈팅, FAQ 문서가 여기에 해당해요.

문서의 목적을 "퍼널 구현이 복잡해질 때 이 문서를 읽고 서브 퍼널을 만들어 퍼널을 효율적으로 관리하는 방법을 배울 수 있다."로 정했다면 **How-to 가이드**로 만들면 돼요.  
문제 해결 문서는 문제 상황 → 원인 → 해결 방법 순서로 구성하는 게 효과적이에요.

### 필요한 기능이나 정보를 빠르게 찾아볼 때

**참조를 위한 문서**를 선택하세요. 참조를 위한 문서는 독자가 빠르게 특정 기능의 사용법이나 속성, 옵션 등을 확인할 때 유용해요. 
함수나 기능 레퍼런스, API 레퍼런스 문서가 이 유형에 포함돼요.

예를 들어 문서의 목적을 "es-toolkit이 제공하는 함수와 속성을 찾아 사용 방법을 확인할 수 있다."로 정했다면 **레퍼런스 문서**로 작성하면 돼요.  

참조 문서는 일반적으로 페이지에 들어갈 내용이 정해져 있어요. API 문서의 경우 요청 메서드, 엔드포인트, 파라미터, 응답 값, 예제 순으로 구성하면 독자가 빠르게 정보를 탐색할 수 있어요. 

### 개념이나 원리를 깊이 이해하고 싶을 때
...
문서 유형을 정했다면 이제 문서 구조를 설계해요. 독자는 정해진 문서 구조를 따라 탐색하고 학습하기 때문에 중요한 과정이에요.
